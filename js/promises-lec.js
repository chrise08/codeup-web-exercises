...
const listNames = (arr => arr.forEach(element => console.log(element.name))); // way to create a reusable function to get the array element names

pokemonPromise.then(response => response.json())
    // the return from this first function, is passed as a parameter in the second one (next line)
...
starWarsAPI.then(response => response.json())
    ...
    .then(resultsArr => resultsArr.forEach(film => {
...
myPromise.then(response => console.log('resolved!'))
